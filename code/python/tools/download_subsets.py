...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        default="hypersim",
        help="Root directory for downloaded files (default: hypersim)",
    )
    parser.add_argument(
        "--subsets",
        nargs="+",
        required=True,
        help="Subsets to download, e.g. ai_001 ai_002 ai_003",
    )
    args = parser.parse_args()

    # Group URLs by subset
    groups = defaultdict(list)
    for url in URLS:
        filename = os.path.basename(url)
        m = re.match(r"(ai_\d{3})_\d{3}\.zip", filename)
        if m:
            groups[m.group(1)].append(url)
    ...
    for subset in args.subsets:
        if subset not in groups:
            ...
            print("Available subsets:", ", ".join(sorted(groups.keys())))
            continue
        print(f"Downloading {subset} ({len(groups[subset])} files)")
        print(f"Output directory: {args.output_dir}")
        for url in sorted(groups[subset]):
            filename = os.path.basename(url)
            output_path = os.path.join(args.output_dir, filename)
            extract_dir = os.path.join(args.output_dir, os.path.splitext(filename)[0])
            if os.path.exists(output_path):
                print(f"[Skip] ZIP exists: {filename}")
                continue
